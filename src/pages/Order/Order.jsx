...
import { deleteFailedOrder, getAllOrders, updateOrder } from '../../features/actions/order/order';
import OrderViewModal from './OrderViewModal';
import Pagination from './Pagination'; // Path to your pagination component
import { jsPDF } from "jspdf";
import "jspdf-autotable";
...
const Order = () => {
    ...
    // Added pagination from the selector
    const { orderData, isLoading, pagination } = useSelector(state => state.order)
    ...
    // Helper: Check if order was created in the last 30 minutes
    const isNewOrder = (createdAt) => {
        if (!createdAt) return false;
        const orderTime = new Date(createdAt).getTime();
        const now = new Date().getTime();
        const thirtyMinutesInMs = 30 * 60 * 1000;
        return (now - orderTime) < thirtyMinutesInMs;
    };
    ...
    // PDF logic (preserved from your original)
    const doc = new jsPDF();
    doc.text("Monthly Orders Report", 14, 10);
    const columns = [
        { header: "Order Id", dataKey: "orderId" },
    ...
    const rows = Array.isArray(orderData) ? orderData.map((item) => ({
        orderId: item?.orderNumber,
        name: item?.orderBy ? `${item.orderBy.firstName} ${item.orderBy.lastName}` : item?.guestMetaData?.name,
        payAmount: `£${(Number(item?.totalAmount?.total) + Number(item?.totalAmount?.deliveryCharge) - Number(item?.totalAmount?.discountPrice || 0)).toFixed(2)}`,
        time: item?.time,
        orderType: item?.orderType,
    ...
    useEffect(() => {
        if (Array.isArray(orderData) && orderData.length > 0) {
            ...
            orderData?.forEach((item) => {
                initialApproval[item._id] = item.orderStatus == "Pending" ? "Pending" : item.orderStatus;
            });
            setApproval(initialApproval);
        }
    }, [orderData]);
        ...
        <>
            <div className="max-w-screen-xl mx-auto px-4 md:px-8 mt-4">
                <div className="items-start justify-between md:flex">
                    ...
                    <div className='flex gap-3'>
                        <select className='border rounded px-2'>Date</select>
                        <button className='text-white bg-blue-700 px-2 py-1 rounded-md hover:bg-blue-600' onClick={() => doc.save("a4.pdf")}>
                            Download PDF
                        </button>
                    </div>
                ...
                <div className="mt-6 shadow-xl rounded-lg overflow-x-auto bg-white">
                    <table className="w-full table-auto text-sm text-left">
                        <thead className="bg-gray-50 text-gray-600 font-medium border-b">
                            <tr>
                                <th className="py-3 px-3">Order Id</th>
                        ...
                        <tbody className="text-gray-600 divide-y">
                            {isLoading ? (
                                ...
                                Array.isArray(orderData) && orderData.length > 0 && orderData.map((item, idx) => (
                                    <tr key={idx} className='hover:bg-slate-100 cursor-pointer' onClick={() => !stopViewModal && handleViewModal(item)}>
                                        <td className="px-3 py-4 whitespace-nowrap">
                                        ...
                                        <td className=" py-4 whitespace-nowrap ">
                                            <span className={`rounded-md py-1 px-3 font-bold capitalize ${item?.paymentMethode === 'Online Payment' ? "text-emerald-600" : "text-yellow-600"}`}>
                                                {item?.orderType === "collection" && item?.paymentMethode === "Cash on delivery" ? "Pay on Collection" : item?.orderType === "delivery" && item?.paymentMethode === "Cash on delivery" ? "Pay on delivery" : item?.paymentMethode}
                                            </span>
                                        </td>
                                        <td className="px-3 py-4 whitespace-nowrap ">
                                            <form onSubmit={(e) => handleSubmit(e, item._id)} className="flex items-center">
                                                <select
                                                    onMouseOver={() => setStopViewModal(true)}
                                                    onMouseLeave={() => setStopViewModal(false)}
                                                    value={approval[item._id] || ""}
                                                    onChange={(e) => handleChange(e, item._id)}
                                                    className="px-3 py-2 text-sm text-gray-600 bg-white border rounded-lg"
                                                >
                                                    <option value="" disabled hidden>Status</option>
                                                    <option value="Completed">Completed</option>
                                                    <option value="Cancelled">Cancelled</option>
                                                </select>
                                                <button type="submit" className="py-2 px-3 font-semibold text-blue-500 hover:text-blue-600">Save</button>
                                            </form>
                                        </td>
                                        <td className=" whitespace-nowrap">
                                            <button onClick={() => handleViewModal(item)} className="py-2 px-6 font-semibold text-green-600 hover:text-green-700">
                                                View
                                            </button>
                                        </td>
                                    </tr>
                    ...
                    {!isLoading && pagination && (
                        <Pagination 
                            ...
                            totalItems={Number(pagination.totalCount)} 
